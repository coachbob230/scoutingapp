<?php
	...
	$game = $row['game'];
	$sql = "DELETE FROM interview_attributes WHERE game='$game'";
	...
	$i = 1;
	while(isset($_POST['attribute' . $i])) {
		$attribute = $_POST['attribute' . $i];
		$sql = "INSERT INTO interview_attributes (game, attribute) VALUES ('$game', '$attribute')";
		if ($conn->query($sql) === TRUE) {
