<?php
	...
	if($question != $ogquestion) {
		$sql = "UPDATE interview_questions SET question = '$question' WHERE question = '$ogquestion'";
		if ($conn2->query($sql) === TRUE) {
			echo "Question edited successfully<br/>";
		} else {
			echo "Error editing question: " . $conn2->error . "<br/>";
		}
		$sql = "ALTER TABLE interview_sheet CHANGE COLUMN `".$ogquestion."` `".$question."` VARCHAR(100)";
		echo $sql . "<br/>";
		if ($conn2->query($sql) === TRUE) {
	...
	if($old_type != $question_type && $old_type == 'mc') {
		...
		$i = 1;
		$sql = "UPDATE interview_questions SET num_choices = '0' WHERE question = '$question'";
		if ($conn2->query($sql) === TRUE) {
		...
		while($row3['choice' . $i] != '') {
			$sql = "UPDATE interview_questions SET choice".$i." = '' WHERE question = '$question'";
			if ($conn2->query($sql) === TRUE) {
				echo "Question edited successfully\n";
			} else {
				echo "Error editing question: " . $conn2->error . "\n";
	...
	if($question_type == "mc") {
		...
		$sql = "UPDATE interview_questions SET part_of_game = '$part_of_game', question_type = '$question_type' WHERE question = '$question'";
		if ($conn2->query($sql) === TRUE) {
			echo "Question edited successfully\n";
		} else {
			echo "Error editing question: " . $conn2->error . "\n";
